 ...
 * #1
 *
 * Розробити функцію, яка використовує метод reduce масиву для обчислення
 * суми усіх елементів масиву чисел.
...
*/
// Функция для подвоения значений в массиве
...
class SkillsManager {
  constructor() {
    this.skills = []; // Ініціалізація порожнього масиву для навичок
  }

  addSkill(skill) {
    // Перевірка, чи аргумент є рядком та має мінімум два символи
    if (typeof skill === 'string' && skill.length >= 2) {
      this.skills.push(skill); // Додавання навички до масиву
      return skill; // Повернення доданої навички
    }
    return null; // Повернення null, якщо умови не виконані
  }

  getAllSkills() {
    return this.skills; // Повернення поточного масиву навичок
  }
}
...
function DateCalculator(initialDate) {
  // Ініціалізація початкової дати
  this.date = new Date(initialDate);

  // Метод для додавання днів
  this.addDays = function(days) {
    this.date.setDate(this.date.getDate() + days);
  };

  // Метод для віднімання днів
  this.subtractDays = function(days) {
    this.date.setDate(this.date.getDate() - days);
  };

  // Метод для отримання дати у форматі "YYYY-MM-DD"
  this.getResult = function() {
    const year = this.date.getFullYear();
    const month = String(this.date.getMonth() + 1).padStart(2, '0'); // Місяці у JavaScript від 0 до 11
